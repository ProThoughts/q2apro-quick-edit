				...
				if(!isset($postid) || !isset($posttitle) || !isset($posttags)) {
					...
					if($tableExists) {
						...
						$postDataOld = qa_db_read_one_assoc(qa_db_query_sub('SELECT title,content,tags FROM ^posts
																WHERE `postid`=#
																', $postid), true);
						// question title has been changed, save old data in history
						if($posttitle != $postDataOld['title']) {
						
							// dont log ninja edit (i.e. many edit within a time span of 5 minutes)
							$now = time();
							$lastupdate = qa_db_read_one_value(qa_db_query_sub('SELECT updated FROM ^edit_history
																WHERE `postid`=#
							...
							if(abs($now - $lastupdate) > 300) {
								...
								qa_db_query_sub('INSERT INTO ^edit_history 
													(postid, updated, title, content, tags, userid, reason) 
													VALUES (#, NOW(), $, $, $, #, $)', 
													$postid, $postDataOld['title'], $postDataOld['content'], $postDataOld['tags'], $userid, '' );
							}
						} // end ** edit history **
					}
